...
colors=[(0,255,0,70),(255,0,0,70),(255,255,100,70),(10,255,255,70)]+[(50+np.random.randint(200),50+np.random.randint(200),150+np.random.randint(100),50) for a in range(10)]
...
class decayHandler():
	...
	def addToTable(self,table,counts):
		...
		timeItem = QtGui.QTableWidgetItem();timeItem.setText( '%.2f'%(time.time() - self.start_time) )
		cItem = QtGui.QTableWidgetItem();cItem.setText( '%.2f'%(counts) )
		xItem = QtGui.QTableWidgetItem();xItem.setText( "  X  " );
...
class regionWidget(QtGui.QWidget,ui_regionWidget.Ui_Form):
	def __init__(self,graph,onDelete,calinv,num):
		...
		col = colors[num]
		self.region.setBrush(col)
		self.viewButton.setStyleSheet("background-color: rgba(%d,%d,%d,%d);"%(col[0],col[1],col[2],255))
	...
	def regionChanged(self,region=None):
		if type(region) == pg.graphicsItems.LinearRegionItem.LinearRegionItem:
			...
			self.regionStart.setValue(start)
			self.regionStop.setValue(stop)
		else:
	...
	def viewData(self):
		...
		#	return
		self.info.show()
		self.info.setWindowState(QtCore.Qt.WindowActive)
...
class AppWindow(QtGui.QMainWindow, decayLayout.Ui_MainWindow):
	def __init__(self, parent):
		...
		self.X = [];self.temp_X = []
		self.Y = [];
		self.dY = [];self.temp_dY = [];self.fit_dY = []
	...
	def mouseMoved(self,evt):
		...
		if self.plot.sceneBoundingRect().contains(pos):
			...
			if index >= 0 and index < len(self.fit_dY):
				self.plot.plotItem.titleLabel.setText("<span style='font-size: 12pt'>x=%0.1f,   <span style='color: red'>counts=%d</span>,   <span style='color: green'>fitted=%.1f</span>" % (mousePoint.x(), self.temp_dY[index], self.fit_dY[index]))
	...
	def fitAndPlot(self):
		if len(self.temp_X)>4: #Atleast 4 points are necessary to attempt an exponential fit.
			try:
				...
				#print A,K
				self.plot.plotItem.titleLabel.setText(" <span style='color: green'>N0 = %.2f , Decay Coefficient = %.2e, Half life = %.2f Min </span>"%(A,K,0.693/K/60))
				self.fitcurve.setData(self.temp_X,self.fit_dY)
			except Exception as e:
				print( e)
		else:
	...
	def openFile(self):
		path = QtGui.QFileDialog.getOpenFileName(self, 'Open half life data file', '.', "Data files (*.csv *.txt)")
		if path:
			path = str(path)
			with open(path) as f:
				header = f.readline()
