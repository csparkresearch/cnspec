...
from .templates import ui_fileBrowser
...
class fileBrowser(QtWidgets.QFrame, ui_fileBrowser.Ui_Form):
    ...
    def itemClicked(self, sel):
        ...
        self.comments.append(
            '''<p style=" margin-top:0px; margin-bottom:0px; margin-left:0px; margin-right:0px; -qt-block-indent:0; text-indent:0px;"><span style=" font-size:9pt; color:#058879;">%s</span></p>\n''' % fname)
    ...
    def browseFile(self):
        filename = QtWidgets.QFileDialog.getOpenFileName(self, " Open a histogram data file", "",
    ...
    def browseListFile(self):
        filename = QtWidgets.QFileDialog.getOpenFileName(self, " Open a List Mode data file", "",
                                                     "Data Files (*.txt *.csv *.dat)")
        if filename:
            pcs = filename[0].split('.')
            if pcs[-1] in self.allowed_extensions:  # check if extension is acceptable
    ...
    def changeDirectory(self):
        dirname = QtWidgets.QFileDialog.getExistingDirectory(self, "Load a folder containing data",
                                                         os.path.expanduser(self._browserPath),
    ...
    def loadProject(self):
        ...
        import fnmatch
        datadir = QtWidgets.QFileDialog.getExistingDirectory(self,
                                                         "Load a folder containing timestamped files -> data_xxmins.txt/csv ",
                                                         os.path.expanduser(self._browserPath))
        if datadir:
        ...
        for a in files_to_show:
            ...
            if pcs[-1] in self.allowed_extensions:  # check if extension is acceptable
                ...
                if not os.path.exists(thumbpath):  # need to create fresh thumbnail
                    try:
                        self.pathLabel.setText('Generating thumbnail for %s' % (filepath));
                        self.app.processEvents()
                        map(os.remove, glob.glob(os.path.join(thumbdir,
                                                              fname) + '*.' + thumbFormat))  # remove old thumbnails (different timestamp) if any
                        ...
                        continue
                try:
                    self.pathLabel.setText('Loading thumbnail for %s' % (filepath));
                    self.app.processEvents()
                    # print ('hm : Loading thumbnail for %s'%(filepath))
                    x = QtGui.QIcon(thumbpath)
                    a = QtWidgets.QListWidgetItem(x, fname)
                    self.listWidget.addItem(a)
                    self.thumbList[fname] = [a, filepath]
                except Exception as e:
                    print('failed to load thumbnail for ', fname, e)
    ...
    def showNewPlot(self, fname, **kwargs):
        ...
        for name, col in zip(self.trace_names, self.trace_colors):
            if kwargs.get('fit', False):
                C = pg.PlotCurveItem(name=name, pen=col, symbolBrush=(0, 0, 200), symbolPen='w', symbol='o',
                                     symbolSize=14)
            else:
        ...
        self.loadFromFile(self.p1, self.newcurves, fname, fit=kwargs.get('fit', False), log=kwargs.get('log', False),
        ...
        for A in range(int(len(ar[0]) / 2)):  # integer division
            ...
            if kwargs.get('histMode', False):
                ...
                if A < len(self.trace_colors):
                    ...
                    brush = (0, 20, 255, 130)
                if A < len(curves): curves[A].setData(self.x, self.y[:-1], stepMode=True, fillLevel=0, brush=brush,
                                                      pen=pen)
            else:
                if fit:  # Exponential decay fitting.
                    ...
                    fitcurve = pg.PlotCurveItem(
                        name=" <span style='color: green'>N0 = %.2f , Decay Coefficient = %.2e, Half life = %.2f Min </span>" % (
                        Amp, K, 0.693 / K), pen=curves[A].opts['pen'])
                    plot.addItem(fitcurve)

                    print('N0=%.2f ; Residual=%.3f SD of slope (log mode) = ' % (Amp, res), np.sqrt(
                        (1. / (len(self.x) - 2)) * sum((log_Amp + K * self.x - np.log(self.y)) ** 2) / sum(
                            (self.x - np.average(self.x)) ** 2)))
                ...
                if fit:
                    tmpc = plot.plot(self.x, Y, pen=None, symbolBrush=(0, 100, 200), symbolPen='b', symbol='star',
                                     symbolSize=12)
                else:
